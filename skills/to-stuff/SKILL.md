...
5. **Validate.** In the repository run `npm run build && npm run check`. Inspect the generated `dist/index.html`, `dist/z/index.html`, and `dist/<slug>/index.html`. Verify the root contains only the `stuff.` wordmark, the hidden `/z/` index links directly to the detail page, and the detail page contains no Stuff chrome or route back to `/` or `/z/`.
...
7. **Verify production.** Confirm local and remote commit hashes match. Poll `https://stuff.zaynjarvis.com/`, `https://stuff.zaynjarvis.com/z/`, and `https://stuff.zaynjarvis.com/<slug>/` for up to two minutes. The root must contain the `stuff.` wordmark without an index link, `/z/` must contain the title, and the detail route must return HTTP 200. Use a query such as `?v=<commit>` to avoid stale cache.
8. **Report.** Return the public detail URL, hidden index URL, GitHub commit, and validation result. State any unverified gap plainly.
...
- Reserve the slug `z` for the hidden artifact index.
- Treat artifact pages as standalone destinations. Never inject a Stuff header, back button, or route to `/` or `/z/`.
